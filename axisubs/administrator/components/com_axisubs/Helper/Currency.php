...
class Currency {
	 ...
	 */
      ...
      $currency_model = $container->factory->model('Currencies');
			
      $rows = $currency_model->limitstart(0)->enabled(1)->get()->toArray();
		...
		if (isset($currency) && (array_key_exists($currency, $this->currencies))) {
			$this->set($currency);
    	} elseif ($this->session->has('currency', 'axisubs') && (array_key_exists($this->session->get('currency', '', 'axisubs'), $this->currencies))) {
      		$this->set($this->session->get('currency', '', 'axisubs'));
    	} else {
      		$this->set(Axisubs::config()->get('config_currency'));
    	}
  	}
  	...
  	public function set($currency) {
    	...
    	if (!$this->session->has('currency', 'axisubs') || ($this->session->get('currency', '', 'axisubs') != $currency)) {
      		$this->session->set('currency', $currency, 'axisubs');
    	}
  	}
  	...
  	public function getId($currency = '') {
		if (!$currency) {			
			return $this->currencies[$this->code]['axisubs_currency_id'];
		} elseif ($currency && isset($this->currencies[$currency])) {
			return $this->currencies[$currency]['axisubs_currency_id'];
		} else {
			return 0;
		}
  	}
