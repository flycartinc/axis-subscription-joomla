...
namespace Flycart\Axisubs\Admin\Render;
 ...
 */
class AxisRenderer extends RenderBase implements RenderInterface
{
	/**
...
HTML;
		...
		$show_header_html		 = $form->getAttribute('show_header_html', 1);
		...
		if ($count = count($items))
		{
			...
			foreach ($items as $k => $item)
			{
				...
				foreach ($fields as $field)
				{
					...
					$oneField_name = $field->name;
					$one_readable_row->$oneField_name = $field->getRepeatable();
			...
			$view->readable_items = $readable_items;
			$default_items_html = '';
			try {
				$default_items_html = $view->loadAnyTemplate('admin:com_axisubs/'.$view->getName().'/default_items');
			} catch (\Exception $e) {
				$default_items_html ='';
			}
